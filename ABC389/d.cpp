...
const vector<long double> dx = {0.5,0.5,-0.5,-0.5};
const vector<long double> dy = {0.5,-0.5,0.5,-0.5};

bool checker(ll i, ll j, ll n){
    long double r = n;
    long double l;
    rep(k,4){
        l = (i + dx[k]) * (i + dx[k]) + (j + dy[k]) * (j + dy[k]);
        l = sqrt(l);
        if(l > r) return true;
    }
    return false;
...
int main(){
    ...
    for(ll i = 1; i < n; i++){
        ...
        while(right - left > 1){ //rightは配列[mid]より小さい最大値に、leftは配列[mid]より大きい最小値になる
            ...
            if(checker(mid,i,n)){ //配列[mid]より比較対象が小さい場合
                right = mid;
            }else{
                left = mid;
            }
