...
int main(){
    ...
    while(true){
        if(pq.empty()) break;
        auto [point,point2] = pq.top();
        pq.pop();
        bool flag = false;
        if(now % x != 0) flag = true;
        if(point >= ((now / x) + flag)) break;
        if(used[point2.fi][point2.se]) continue;
        now += point;
        used[point2.fi][point2.se] = true;
        rep(4){
            ll nx = point2.fi + dx[i];
            ll ny = point2.se + dy[i];
            if(nx < 0 || nx >= h || ny < 0 || ny >= w) continue;
