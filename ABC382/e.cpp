...
int main(){
    ...
    rep(n) cin >> a[i];
    rep(n) a[i] /= 100;
    vector<vector<long double>> dp(x+1000, vector<long double>(n+1000, 0));
    dp[0][0] = 1;
    priority_queue<tuple<ll, ll, long double>, vector<tuple<ll, ll, long double>>, greater<tuple<ll, ll, long double>>> pq;
    pq.push({0,0,1});
    long double ans = 0;
    while(!pq.empty()){
        auto [i, j, p] = pq.top();
