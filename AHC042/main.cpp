...
int main() {
    ...
    while(clock() - timer < 1.6 * CLOCKS_PER_SEC && !demons.empty()) { 
        ...
        if (up_count <= down_count && up_count <= left_count && up_count <= right_count && can_move_up) {
            ...
            bool flag = true;
            rep(k,n){
                if(c[k][j] == 'o') flag = false;
                //if(c[k][j] == 'x') flag = false;
            }
            if(!flag){
            ...
            bool flag = true;
            rep(k,n){
                if(c[n-k-1][j] == 'o') flag = false;
                //if(c[n-k-1][j] == 'x') flag = false;
            }
            if(!flag){
            ...
            bool flag = true;
            rep(k,n){
                if(c[i][k] == 'o') flag = false;
                //if(c[i][k] == 'x') flag = false;
            }
            if(!flag){
    ...
    while(!demons.empty() && (clock() - timer) / CLOCKS_PER_SEC < 1) {
          // 鬼があるマス
        ...
        if (up_count <= down_count && up_count <= left_count && up_count <= right_count && can_move_up) {
            ...
            rep(k, up_count) prove.emplace_back('U', j);
            rep(k, up_count) move('U', j, c);
        } else if (down_count <= up_count && down_count <= left_count && down_count <= right_count && can_move_down) {
            rep(k,n){
                if(c[n-k-1][j] == 'o') break;
            ...
            rep(k, down_count) prove.emplace_back('D', j);
            rep(k, down_count) move('D', j, c);
        } else if (left_count <= right_count && left_count <= up_count && left_count <= down_count && can_move_left) {
            rep(k,n){
                if(c[i][k] == 'o') break;
            ...
            rep(k, left_count) prove.emplace_back('L', i);
            rep(k, left_count) move('L', i, c);
        } else if (can_move_right) {
            rep(k,n){
                if(c[i][n-k-1] == 'o') break;
            ...
            rep(k, right_count) prove.emplace_back('R', i);
            rep(k, right_count) move('R', i, c);
        }
        demons.clear();
        rep(i, n){
    ...
    if (flag) {
        rep(i, prove.size()) {
            cout << prove[i].first << " " << prove[i].second << "\n";
        }
    }else{
