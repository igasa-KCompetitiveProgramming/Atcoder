...
const ll dx[4] = {1,0,-1,0}; const ll dy[4] = {0,1,0,-1};

ll ans = -1;

ll dfs(ll y, ll x, ll d ,ll h, ll w, vector<string> s, ll gy, ll gx){
    if(y == gy && x == gx && d != 0){
        if(d < 3) return 0;
        ans = max(ans,d);
        return d;
    }
    if(y < 0 || y >= h || x < 0 || x >= w || s[y][x] == '#') return 0;
    s[y][x] = '#';
    rep(4){
        ll ny = y + dy[i];
        ll nx = x + dx[i];
        dfs(ny,nx,d+1,h,w,s,gy,gx);
    }
    return d;
}
