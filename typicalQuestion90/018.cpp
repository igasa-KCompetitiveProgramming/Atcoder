...
ld pi = 3.141592653589793238462643383279502884197169399375105820974944592307816406286;
int main(){
    ld t,l,x,y,q;
    cin >> t >> l >> x >> y >> q;
    vector<ld> e(q);
    ...
    rep(i,q){
        ld nx = 0;
        ld ny = l/2*cos(-2*pi*e[i]/t - pi/2);
        ld nz = l/2*sin(-2*pi*e[i]/t - pi/2) + l/2;
        ld ans = atan2(nz,sqrt(x*x + (y-ny)*(y-ny)));
