...
int main(){
    LL(n);
    vector<pll> h(n), w(n);
    rep(n) cin >> w[i].fi >> h[i].fi >> w[i].se >> h[i].se;
    v(ans, n, 0);
    vv(ruiseki, 1010, 1010, 0);
    rep(i,n){
        ruiseki[h[i].fi][w[i].fi]++;
        ruiseki[h[i].fi][w[i].se]--;
        ruiseki[h[i].se][w[i].fi]--;
        ruiseki[h[i].se][w[i].se]++;
    }
    
    rep(i,1010){
        rep(j,1010){
            if(j != 0) ruiseki[i][j] += ruiseki[i][j-1];
        }
    }
    rep(i,1010){
        rep(j,1010){
            if(i != 0) ruiseki[i][j] += ruiseki[i-1][j];
        }
    }

    rep(i,1010){
        rep(j,1010){
            if(ruiseki[i][j] > 0) ans[ruiseki[i][j]-1]++;
        }
    }
