...
int main(){
    ...
    while(!que.empty()){
        pll now = que.back();
        que.pop_back();
        if(used[now.fi]) continue;
        length[now.se].pb(now.fi);
        used[now.fi] = true;
        rep(g[now.fi].size()){
            if(used[g[now.fi][i]]) continue;
            que.pb({g[now.fi][i], now.se+1});
        }
    }
    ll cnt = 0;
    ll tmp = 0;ll res = 0;
    ...
    rep(i,0,n,2){
        rep(j,length[i + flag].size()){
            cout << length[i + flag][j] + 1 << spa;
            cnt++; if(cnt == n/2) return 0;
        }
    }
}