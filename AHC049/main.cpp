  ...
  public:
    ...
    void twoBoxesHas(){
        ...
        while(!pq.empty()){
            ...
            //cout << durability << spa << now.fi << spa << now.se << endl;

            while(true){
                pint next = {-1, -1};
                int maxDistance = -1;
                rep(i, max(now.fi, (ll)1)){
                    rep(j, max(now.se, (ll)1)){
                        //cout << i << spa << j << endl;
                        if(i == now.fi && j == now.se) continue;
                        if(i == 0 && j == 0) continue;
                        if(field[i][j].fi == -1) continue;
                        if(durability - field[i][j].fi * (i + j) <= 0) continue;
                        int distance = i + j;
