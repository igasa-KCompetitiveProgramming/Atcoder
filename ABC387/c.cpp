        ...
        rep(j,1,10){
            ...
            if(tmp <= l && l <= res){
                ...
                rep(k,s.size() - 1){
                    ...
                    if((checker - (s[k] - '0') - 1) * power(checker,s.size()-k-1) < 0){
                        flag2 = false;
                        break;
                    }
                    ans += max(0ll,(checker - (s[k] - '0') - 1) * power(checker,s.size()-k-1));
                }
                if(flag2) ans += max(0ll,checker - ll(s[s.size()-1] - '0'));
            }else if(flag){
                string s = to_string(tmp);
                ans += power(j,s.size() - 1);
            }
            if(tmp <= r && r <= res){
                flag = false;
                ...
                if(flag2) ans -= max(0ll,checker - (s[s.size()-1] - '0') - 1);
                finish = true;
                break;
            }
        }
