...
ll solve(ll h, ll w, ll d, pll a, pll b, vector<string> s){
    vector<string> t(h, string(w, '.'));
    //cout << a.fi << spa << a.se << spa << b.fi << spa << b.se << endl;
    rep(t.size()){
        rep(j,t[i].size()){
            if(s[i][j] == '#') continue;
            if(abs(i - a.fi) + abs(j - a.se) <= d || abs(i - b.fi) + abs(j - b.se) <= d){
                //cout << i << spa << j << endl;
                t[i][j] = '#';
            }
        }
...
int main(){
    ...
    vector<pll> pos;
    ll ans = 0;
    rep(h) rep(j,w) if(s[i][j] == '.') pos.pb({i,j});
    rep(pos.size()){
        rep(j,i+1,pos.size()){
            ans = max(ans, solve(h,w,d,pos[i],pos[j],s));
        }
    }
    O(ans);
}