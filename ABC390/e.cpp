...
int main(){
    LL(n,x);
    vector<vector<pll>> a(3);
    rep(n){
        LL(v,b,c);
        a[v-1].pb({b,c});
    }
    vector<vector<ll>> dp(3,vector<ll>(x+1,0));
    rep(3){
        rep(j,a[i].size()){
            for(int k = x;k>=a[i][j].se;k--){
                dp[i][k] = max(dp[i][k], dp[i][k-a[i][j].se] + a[i][j].fi);
            }
        }
    }
    rep(3){
    ...
    vector<ll> index(3,0);
    rep(i,x){
        if(dp[0][index[0]] <= dp[1][index[1]] && dp[0][index[0]] <= dp[2][index[2]]) index[0]++;
        else if(dp[1][index[1]] <= dp[2][index[2]]) index[1]++;
        else index[2]++;
    }
    cout << min({dp[0][index[0]],dp[1][index[1]],dp[2][index[2]]}) << endl;
}