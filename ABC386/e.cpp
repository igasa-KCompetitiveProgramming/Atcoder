...
void combination(ll id, ll k, bitset<60> bit){
    rep(i,id,a.size() - k+1){
        bitset<60> bit2;
        bit2 = bit ^ a[i];
        //cout << bit2.to_ulong() << spa << k << endl;
        if(k == 1){
            ans = max(ans, (ll)bit2.to_ullong());
        }else{
            combination(i+1, k-1, bit2);
        }
    }
}
...
int main(){
    ...
    if(k == n){
        rep(n) bit2 ^= a[i];
        cout << bit2.to_ullong() << endl;
        return 0;
    }else if(k > n - k){
        rep(n) bit2 ^= a[i];
        combination(0, n-k, bit2);
    }else{
        combination(0, k, bit2);
    }
    cout << ans << endl;
}