...
int main(){
    LL(n,k);
    vector<vector<ll>> num(3,vector<ll>(n,-1));
    rep(3) rep(j,n) cin >> num[i][j];
    rep(3) sort(num[i].rbegin(),num[i].rend());
    map<tuple<ll,ll,ll>,bool> mp;
    priority_queue<tuple<ll,ll,ll,ll>> pq;
    pq.push(make_tuple(calc(num[0][0],num[1][0],num[2][0]),0,0,0));
    mp[make_tuple(0,0,0)] = true;
    rep(k - 1){
        auto [_,a,b,c] = pq.top();
        pq.pop();
        if(a+1 != n && !mp[make_tuple(a+1,b,c)]) pq.push(make_tuple(calc(num[0][a+1],num[1][b],num[2][c]),a+1,b,c));
        if(b+1 != n && !mp[make_tuple(a,b+1,c)]) pq.push(make_tuple(calc(num[0][a],num[1][b+1],num[2][c]),a,b+1,c));
        if(c+1 != n && !mp[make_tuple(a,b,c+1)]) pq.push(make_tuple(calc(num[0][a],num[1][b],num[2][c+1]),a,b,c+1));
        mp[make_tuple(a+1,b,c)] = true;
        mp[make_tuple(a,b+1,c)] = true;
        mp[make_tuple(a,b,c+1)] = true;
    }
    auto [ans,_,__,___] = pq.top();
    O(ans);
}